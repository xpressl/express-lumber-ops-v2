...
export async function createImportJob(input: {
  ...
  const job = await prisma.importJob.create({
    data: { ...input, status: "UPLOADING", createdBy: actorId },
  });

  await createAuditEvent({
    actorId, actorName: "System", action: "import.created",
...
/** Approve an import */
export async function approveImport(jobId: string, actorId: string) {
  ...
  await createAuditEvent({
    actorId, actorName: "System", action: "import.approved",
...
/** Reject an import */
export async function rejectImport(jobId: string, actorId: string) {
  const job = await prisma.importJob.update({
    where: { id: jobId },
  ...
  await createAuditEvent({
    actorId, actorName: "System", action: "import.rejected",
    entityType: "ImportJob", entityId: jobId,
    locationId: job.locationId,
  });
...
/** List import jobs */
export async function listImports(params: { type?: string; status?: string; locationId?: string }) {
...
export async function getImportById(jobId: string) {

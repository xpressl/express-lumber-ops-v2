...
const columns: DataTableColumn<ImportRow>[] = [
  { id: "file", header: "File", cell: (r) => <span className="text-sm truncate max-w-48 block">{r.fileName}</span>, sortable: true },
  ...
  { id: "rows", header: "Rows", cell: (r) => <span className="font-mono text-xs">{r.totalRows}</span>, className: "text-center" },
  { id: "created", header: "Created", cell: (r) => <span className="font-mono text-xs text-success">{r.createdRows}</span>, className: "text-center" },
  { id: "errors", header: "Errors", cell: (r) => <span className={`font-mono text-xs ${r.errorRows > 0 ? "text-destructive" : ""}`}>{r.errorRows}</span>, className: "text-center" },
  { id: "confidence", header: "Confidence", cell: (r) => r.confidenceScore !== null
    ? <span className={`font-mono text-xs ${Number(r.confidenceScore) >= 0.85 ? "text-success" : Number(r.confidenceScore) >= 0.7 ? "text-warning" : "text-destructive"}`}>{(Number(r.confidenceScore) * 100).toFixed(0)}%</span>
    : <span className="text-muted-foreground">—</span>, className: "text-center" },
];
